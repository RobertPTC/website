...
import Storage, { PomodorosForDateRequest } from "app/storage";
...
export default function StackedBarChartDate() {
  ...
  if (!max || !bars) return <></>;
  const now = dayjs();
  const xScaleRange = numberOfHours;
  const chartHeading = `${now.format("MMMM DD, YYYY")}`;
